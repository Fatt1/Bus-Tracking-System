...
const ScheduleAddEditPageNew = () => {
  ...
  useEffect(() => {
    const fetchDropdownData = async () => {
      ...
      try {
        ...
        const allDrivers = Array.isArray(driverResponse.data)
          ? driverResponse.data
          : [];
        ...
        const allBuses = Array.isArray(busResponse.data)
          ? busResponse.data
          : [];
        setBuses(allBuses);
        ...
        console.log(
          "Available drivers:",
          allDrivers.filter((d) => d.canClickable).length
        );
        console.log(
          "Available buses:",
          allBuses.filter((b) => b.canClickable).length
        );
      } catch (error) {
  ...
  const handleSave = async (e) => {
    ...
    if (selectedDriver && !selectedDriver.canClickable) {
      ...
      alert(
        `⚠️ Tài xế "${selectedDriver.driverName}" đã có lịch trình khác trong ngày này. Vui lòng chọn tài xế khác.`
      );
      return;
    }
    ...
    if (selectedBus && !selectedBus.canClickable) {
      ...
      alert(
        `⚠️ Xe buýt "${selectedBus.busName}" đã có lịch trình khác trong ngày này. Vui lòng chọn xe buýt khác.`
      );
      return;
    }
    ...
    try {
      ...
      if (response.data && response.data.id) {
        ...
        console.warn(
          "This might indicate the schedule was not saved to database."
        );
      }
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        if (response.data && response.data.isSuccess === false) {
          console.error(
            "❌ Backend returned success status but isSuccess=false!"
          );
          ...
          {isLoadingDropdowns ? (
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                {drivers.length > 0 &&
                  drivers.every((d) => !d.canClickable) && (
                    <small
                      style={{
                        color: "#e74c3c",
                        marginTop: "4px",
